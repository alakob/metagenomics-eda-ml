...
---
\LRCornerWallPaper{0.4}{logo2.png}
_Blaise Alako_  

__First dataset:Tara Ocean dataset:__
...
suppressWarnings(suppressMessages(require(ggalt)))    # 
#devtools::install_github("hrbrmstr/ggalt")
suppressWarnings(suppressMessages(require(ggthemes))) # theme_map and tableau colors
...
ERP001736 <- "https://www.ebi.ac.uk/metagenomics/projects/ERP001736/download/2.0/export?contentType=text&exportValue=taxonomy_abundances"
ERP001736.meta <- "https://www.ebi.ac.uk/metagenomics/projects/ERP001736/overview/doExport";
taraOcean <- tbl_df(read.delim(file=ERP001736))
meta <-  tbl_df(read.csv(file=ERP001736.meta))
...
taraOcean.long  <- taraOcean %>% 
                      gather(runid, freq, ERR164407:ERR599176) %>%
                        filter(taxonomy !="Root") %>% mutate(runid=as.factor(runid))

```

```{r}
# Parse the sample description for geospatial coordinates, depth of collection, 
# collection sea-layer, date of collection and the Run id from the metadata
meta <- meta %>% select(Sample.Description, Run.ID, Sample.ID)

#meta <- tarainfo(meta=meta)
```

```{r}

getMetadataFromENA <- function (df=df){
  sampleid=df$Sample.ID
  sampleids = paste(sampleid, collapse=",")
  runid=df$Run.ID
  url <- "https://www.ebi.ac.uk/ena/data/view/" 
  url <- paste(url,paste(sampleids, "&display=xml", sep="" ), sep="")
  doc <- xmlParse(getURL(url))
  tags <- sapply(getNodeSet(doc, "//SAMPLE_ATTRIBUTE/TAG"), xmlValue)
  values <- sapply(getNodeSet(doc, "//SAMPLE_ATTRIBUTE/VALUE"), xmlValue)
  info = data.frame(runid=runid, sampleid=sampleid, tag=tags, value=values)
  info <- tbl_df(info)
  info <- info %>% mutate(tag=gsub(" |-", "_", tag, perl=TRUE)) %>% mutate(tag=gsub("\\)|\\(", "", tag, perl=TRUE)) %>% spread(tag, value)
  return(info)
}
...
```{r}
#What information was collect from the audisey. Extract the metadata from European Nucleotide Archive:

metadata <- tbl_df(meta) %>% getMetadataFromENA()
metadata %>% colnames()
```

```{r}

meta.sub <- metadata %>% rename(Latitude=Latitude_Start, Longitude=Longitude_Start) %>%
                        select(runid, sampleid, Latitude, Longitude, 
                                Temperature, Depth , Environment_Feature)

meta.sub <- meta.sub %>% mutate(Latitude            = as.numeric(as.character(Latitude)),
                                Longitude           = as.numeric(as.character(Longitude)), 
                                Temperature         = as.numeric(as.character(Temperature)), 
                                Depth               = as.numeric(as.character(Depth)),
                                Environment_Feature = gsub("\\s*\\(ENVO:.*?\\)\\s*","",Environment_Feature, perl=TRUE)) 

# Append metadata to the the Tara Ocean dataset
taraOcean.long <- left_join(taraOcean.long, meta.sub, by = c("runid" = "runid"))
...
baseplot <- ggplot()
baseplot <- baseplot + geom_map(data=world, map=world,
                    aes(x=long, y=lat, map_id=region),
                    color="white", fill="#7f7f7f", size=0.05, alpha=1/4)

baseplot

baseplot <- baseplot+ geom_point(data=meta.sub, 
                      aes(x=Longitude, y=Latitude , color=Environment_Feature), 
                      size=2, alpha=0.6)
baseplot
baseplot <- baseplot + scale_color_tableau()
baseplot <- baseplot + coord_proj("+proj=wintri")
baseplot <- baseplot + facet_wrap(~Environment_Feature)
baseplot <- baseplot + theme_map()
baseplot <- baseplot + theme(strip.background=element_blank())
...
meta.count <- meta.sub %>% group_by(Environment_Feature) %>%
              mutate(count=n()) %>% ungroup() %>%
              select(Environment_Feature, count)  %>% unique()

meta.count %>% ggplot(aes(x=Environment_Feature, y=count)) +  geom_bar(stat='identity') + 
                geom_text(aes(label=count))  + theme_bw() +
                      theme(axis.text.x=element_text(angle=45, hjust=1)) + 
                      ggtitle("Tara Ocean Dataset")
...
taraOcean.wide1 <- taraOcean.long %>% select(taxonomy, runid,freq) %>%
                    spread(runid, freq, fill=0) %>%
                      as.data.frame()
runs <- taraOcean.long %>% select(taxonomy, runid, freq)
temperature <- taraOcean.long %>% select(taxonomy, runid, freq)
```

```{r}
taraOcean.mat <- taraOcean.df %>% select(-c(1)) %>% as.matrix()
rownames(taraOcean.mat) <- gsub("\\[|\\]","", perl=TRUE, taraOcean.df$taxonomy)
```

```{r}

# Clean up the matrix
taraOcean.mat <- taraOcean.mat[!rownames(taraOcean.mat)=="",]
as.matrix(table(rownames(taraOcean.mat)))[as.matrix(table(rownames(taraOcean.mat)))>1,]
todiscard <- (as.matrix(table(rownames(taraOcean.mat)) > 1))
myselect <- names(todiscard[todiscard[,1]==TRUE,])
taraOcean.mat<- taraOcean.mat[!rownames(taraOcean.mat) %in% myselect,]
taraOcean.mat <- taraOcean.mat[which(rowSums(taraOcean.mat)!=0),]

```
_Is there any association between the species and the data source?_
```{r}
# Perform a chi-square test of independence, this evaluates whethere there is a significant
# dependence between species and biological samples

chisq <- chisq.test(taraOcean.mat)
chisq
...
taraOcean.ca <- CA(taraOcean.mat, graph=FALSE)
```

_Explore the content of the CA output_
```{r}
summary(taraOcean.ca, nb.dec = 2, nbelements = 4, ncp= 3)
...
#          [0-1] of the first 10 active rows variable on the dimension 1 and 2
# Table 3: displays the coordinates, the contribution and the cos2 of the first active column 
#          variables on the dimension 1 and 2
#     
```
_Interprete the CA result above, hint:correlation coef., chi-square statistic_
```{r}
# Correlation coefficient
eig <- get_eigenvalue(taraOcean.ca)
...
# A more rigourous approach for examining the association is to use Chi-square statistics. 
...
fviz_screeplot(taraOcean.ca, barfill='white', addlabels=TRUE) + theme_bw()
...
taraOcean.hcpc <- HCPC(taraOcean.ca, graph = TRUE , nb.clust=4, order=TRUE )
```
...
fviz_ca_biplot(taraOcean.ca, repel=TRUE, select.row = list(contrib = 95), 
...
plot(taraOcean.hcpc, choice ="3D.map", draw.tree = FALSE,
...
res.hcpc <- HCPC(taraOcean.ca, cluster.CA = "columns", graph = TRUE , nb.clust=4, order=TRUE )
```
...
taraOcean.ca <- CA(t(taraOcean.mat), ncp=4)
```

```{r fig.pos="H"}
# Symmetric biplot 
fviz_ca_biplot(taraOcean.ca, repel=TRUE, select.row = list(contrib = 12), 
...
***  
__Second dataset: American Gut Project__  
...
**Pre-requisite**  
1. Create a working directory ("metagenomicWorshop") in your home directory.  

**Set the working environment**  
```{r eval=FALSE}
setwd("/path2/metagenomicWorkshop") 
```
...
```{r}
# merge taxonomy occurence count per data source
gutproject.df <- gutproject.df %>%
...
_Gather all the files with the .tsv extension_  
```{r}
